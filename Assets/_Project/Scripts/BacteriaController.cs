...
public class BacteriaController : MonoBehaviour
{
    ...
    private void CachePlayerReference(bool forceSearch = false)
    {
        ...
        foreach (Collider hit in hits)
        {
            ...
            {
                BindPlayerTarget(hit.gameObject);
                return;
            }
        }
    }

    private void BindPlayerTarget(GameObject target, XRIOfficialPlayerTuning xriTuning = null)
    {
        playerTarget = target;
        playerController = target != null ? target.GetComponent<PlayerController>() : null;
        xriPlayerTuning = xriTuning;

        if (xriPlayerTuning == null && target != null)
        {
            xriPlayerTuning = target.GetComponent<XRIOfficialPlayerTuning>() ??
                target.GetComponentInParent<XRIOfficialPlayerTuning>() ??
                target.GetComponentInChildren<XRIOfficialPlayerTuning>(true);
        }
    }
    ...
    private bool CanHearPlayerMovement(out Vector3 soundTarget, out float heardStrength)
    {
        ...
        if (playerController == null && xriPlayerTuning == null && playerTarget != null)
            BindPlayerTarget(playerTarget);

        bool isProducingFootstepNoise = playerController != null
            ? playerController.IsProducingFootstepNoise
            : xriPlayerTuning != null && xriPlayerTuning.IsProducingFootstepNoise;
        ...
        float movementNoiseStrength = playerController != null
            ? playerController.MovementNoiseStrength
            : xriPlayerTuning.MovementNoiseStrength;
        float baseNoiseStrength = Mathf.Clamp01(movementNoiseStrength);
